...
import { validatePlace, endpointMatches, distanceKm, type Place } from '../../../lib/locations';
...
export async function GET(request: Request) {
    try {
        ...
        if (action === 'rides') {
            ...
            let fromPoint: Place | null = null, toPoint: Place | null = null;
            try { for (const side of ['from', 'to']) { const value = u.searchParams.get(side + 'Point'); if (value) { if (value.length > 1400) throw new Error('Location too long'); const p = validatePlace(JSON.parse(value)); if (side === 'from') fromPoint = p; else toPoint = p } } } catch { return fail('Select valid Karachi locations.', 400) }
            let rides = await all(base + ` WHERE r.date=? AND r.service_city='Karachi' AND r.status='scheduled' AND datetime(r.date || 'T' || r.time || ':00','-5 hours') > datetime('now') ORDER BY r.time LIMIT 200`, date);
            rides = rides.filter(r => (fromPoint || toPoint ? endpointMatches(r.origin_point, fromPoint) && endpointMatches(r.destination_point, toPoint) && (!from || fromPoint || routeMatches(r.origin, r.destination, r.stops, from, '')) && (!to || toPoint || routeMatches(r.origin, r.destination, r.stops, '', to)) : routeMatches(r.origin, r.destination, r.stops, from, to)) && r.occupied < r.seats);
...
export async function POST(request: Request) {
    try {
        ...
        if (action === 'create') {
            ...
            const originPoint = validatePlace(input.originPoint), destinationPoint = validatePlace(input.destinationPoint);
            const origin = originPoint.label, destination = destinationPoint.label; if (distanceKm(originPoint, destinationPoint) < 0.05) return fail('Choose pickup and drop-off points at least 50 metres apart.');
            ...
            await db().batch(futureDates.map(date => stmt('INSERT INTO rides (id,driver,origin,destination,stops,date,time,seats,car,plate,fare,notes,pickup,dropoff,service_city,origin_point,destination_point) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', crypto.randomUUID(), uid, origin, destination, stops, date, time, seats, car, plate, fare, notes, pickup, dropoff, 'Karachi', JSON.stringify(originPoint), JSON.stringify(destinationPoint)))); return json({ ok: true, count: futureDates.length });
        }
